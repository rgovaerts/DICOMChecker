...
<xsl:template match="d:tr" mode="groupnestinglevels">
	...
	<xsl:if test="name(parent::node()) = 'tbody' and $row != ''">	<!-- untidy way to check not header, but it works :(; also excludes unlabelled rows-->
		...
		<xsl:variable name="requirementType"><xsl:value-of select="normalize-space(d:td[7])"/></xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="$valueType = 'INCLUDE'">
				...
				<includetemplate row="{$row}" tid="{$includeTID}" relationship="{$relationship}" vmmin="{$vmmin}" vmmax="{$vmmax}" requiredType="{$requirementType}"/>
			...
			<xsl:otherwise>
				<templatecontentitem>
					...
					<xsl:attribute name="requirementType"><xsl:value-of select="$requirementType"/></xsl:attribute>
